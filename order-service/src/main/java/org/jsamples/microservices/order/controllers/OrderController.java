...
public class OrderController extends Controller<OrderService> {
    ...
    @PostMapping
    public ResponseEntity<Void> add(@RequestBody @Valid OrderVO order, UriComponentsBuilder builder) {
        return this.create(service -> service.add(order), builder.path("/orders/{id}"));
    }
}
